            ...
            if (checkRange(i, j) == 1) {
                for (int k = 0; k < 2; k++) {
                    ...
                    if (board[nx][ny] >= board[i][j]) {
                        int temp = board[nx][ny] + 1 - board[i][j];
                        visited[nx][ny] = min(visited[nx][ny], visited[i][j] + temp);
                    }
                    else{
                        visited[nx][ny] = min(visited[nx][ny], visited[i][j]);
                    }
                }
            }
            else if (checkRange(i, j) == 2){
                int nx, ny;
                nx = i + dx[0]; ny = j + dy[0];
                if (board[nx][ny] >= board[i][j]) {
                    int temp = board[nx][ny] + 1 - board[i][j];
